...
import torch
import numpy as np
from surro_models import resnet, pyramidnet, vgg, densenet
import json
...
def load_adv(model_list = ['pyramidnet']):
    nets = []
    basepath = "./pretrainedClassifiers"
    for model_name in model_list:
        if model_name == "pyramidnet":
            TRAINED_MODEL_PATH = '{}/classification_models/pyramidnet_basic_110_84/00/'.format(basepath)
            ...
            TRAINED_MODEL_PATH = '{}/classification_models/resnet_basic_110/00/'.format(basepath)
            filename = 'model_best_state.pth'
            with open(os.path.join(TRAINED_MODEL_PATH, 'config.json')) as fr:
                pretrained_model = resnet.Network(json.load(fr)['model_config'])
                pretrained_model.load_state_dict(torch.load(os.path.join(TRAINED_MODEL_PATH, filename))['state_dict'])
        elif model_name == 'vgg':
            TRAINED_MODEL_PATH = '{}/classification_models/vgg_15_BN_64/00/'.format(basepath)
            filename = 'model_best_state.pth'
            with open(os.path.join(TRAINED_MODEL_PATH, 'config.json')) as fr:
                pretrained_model = vgg.Network(json.load(fr)['model_config'])
                pretrained_model.load_state_dict(torch.load(os.path.join(TRAINED_MODEL_PATH, filename))['state_dict'])
        elif model_name == 'dense':
            TRAINED_MODEL_PATH = '{}/classification_models/densenet_BC_100_12/00/'.format(basepath)
            filename = 'model_best_state.pth'
            with open(os.path.join(TRAINED_MODEL_PATH, 'config.json')) as fr:
